  ...
  return (
    <main className="mx-auto max-w-4xl p-6">
      <div className="mb-4 flex items-center justify-between">
        ...
        <h1 className="text-xl font-bold text-slate-900">🔍 图片识别内容核对</h1>
        <span className="w-24" />
      </div>
        ...
        <>
          <div className="mb-4 rounded-2xl border border-amber-200 bg-amber-50 p-4">
            <p className="text-sm font-medium text-amber-700">
              ⚠️ 识别置信度较低（{formatConfidence(resume.ocrConfidence)}）
            </p>
            ...
            {blocks.map((block, index) => (
              <div
                ...
                className={`rounded-2xl border bg-white p-4 shadow-sm ${
                  block.confirmed ? "border-green-300" : "border-slate-200"
                }`}
              >
                <div className="mb-2 flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-medium text-slate-400">第 {index + 1} 块</span>
                ...
                {block.reason && (
                  <p className="mb-2 rounded-lg bg-slate-50 px-2.5 py-1.5 text-xs text-slate-400">
                    {block.reason}
                  </p>
                )}
                <textarea
